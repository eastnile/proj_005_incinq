...
setproj(5)
#file.copy(from = paste0(p$proj$remote, 'ipums_acs_100k.csv'), to = p$proj$local, overwrite = T)
#file.copy(from = paste0(p$proj$remote, 'ipums_acs_100k.xml'), to = p$proj$local, overwrite = T)
## Prep meta -----------
setwd('F:/data/ipums cps')
cps.meta <- read_ipums_ddi("cps_00006.xml")
...
gs_edit_cells(ss = gs$meta, ws = 'meta', input = to.gs, anchor = "A1", trim = T, verbose = F)


cps <- as.data.table(read_ipums_micro(ddi))


acs.meta <- read_ipums_ddi("ipums_acs_100k.xml")
acs.meta$var_info$var_name = tolower(acs.meta$var_info$var_name)
saveRDS(acs.meta, 'acsmeta.rds')
## write metadata to gsheets --------
...
recodelist = data.table(gs_read(gs$meta, ws = 'recode', col_types = 'ccc'))
...
rescalelist = data.table(gs_read(gs$meta, ws = 'rescale', col_types = 'cc'))
