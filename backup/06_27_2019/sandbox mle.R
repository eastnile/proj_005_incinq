...
cdf.fit = function(y, fits, plot=T) {
    y = sort(y)
...
fit = fitdistcens(x.censored.fitdist, 'pareto', start = list(shape = 5, scale = 1))
summary(fit)
plot(fit)

fit = fitdist(x.censored, 'pareto', start = list(shape = 5, scale = 1))
summary(fit)
#plot(fit)